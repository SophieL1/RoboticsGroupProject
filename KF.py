...
Q = np.array([[1., 0, 0], [0, 1., 0], [0, 0, 1.]])                          # Process noise covariance (Q)
R = np.array([[0.02, 0, 0], [0, 0.02, 0], [0, 0, 0.2]])                     # Measurement noise covariance (R)
...
### FUNCTIONS ###
def B_kalman(x):
    B = np.array([[math.cos(math.radians(x[2])), 0], [math.sin(math.radians(x[2])), 0], [0, 1]])
    return B
